...
class DecoderTest {
    ...
    private static Stream<Arguments> jsonTestCases() {
        return Stream.of(
                ...
                Arguments.of(
                        ...
                        false),
                Arguments.of(
                        "Non-projectable DecodingFormat",
                        (Supplier<DecodingFormat<DeserializationSchema<RowData>>>)
    ...
    private static final DataType tableDataType =
            DataTypes.ROW(
                    DataTypes.FIELD("a", DataTypes.STRING()), // key field
                    DataTypes.FIELD("b", DataTypes.STRING()), // value field
                    DataTypes.FIELD("c", DataTypes.STRING()), // key field
                    DataTypes.FIELD("d", DataTypes.STRING()), // value field
                    DataTypes.FIELD("e", DataTypes.STRING()), // key field
                    DataTypes.FIELD("f", DataTypes.STRING()) // value field
                    );
    ...
    void testProjectOneMetadataField(
        ...
        final List<String> metadataKeys = List.of("ingestion-timestamp");
        format.applyReadableMetadata(metadataKeys);
    ...
    void testProjectTwoMetadataFields(
        ...
        final List<String> metadataKeys = List.of("ingestion-timestamp", "source.database");
        format.applyReadableMetadata(metadataKeys);
