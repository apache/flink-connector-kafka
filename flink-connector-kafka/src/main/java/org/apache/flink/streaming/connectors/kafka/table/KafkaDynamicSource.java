...
public class KafkaDynamicSource
    ...
    public KafkaDynamicSource(
        ...
        this.projectedFields = tableSchemaProjectedFields(physicalDataType);
        // Kafka-specific attributes
    ...
    public void applyReadableMetadata(List<String> metadataKeys, DataType producedDataType) {
        ...
        if (formatMetadata.size() > 0) {
            this.valueFormatMetadataKeys =
                    formatMetadataKeys.stream()
                            .map(k -> k.substring(VALUE_METADATA_PREFIX.length()))
                            .collect(Collectors.toList());
            valueDecodingFormat.applyReadableMetadata(valueFormatMetadataKeys);
        }
    ...
    private static int[][] tableSchemaProjectedFields(final DataType tableDataType) {
