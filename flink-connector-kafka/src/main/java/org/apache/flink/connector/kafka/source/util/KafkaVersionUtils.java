...
/**
 * Utility class to check Kafka version compatibility and share group feature availability. This
 * ensures proper fallback behavior when share group features are not available.
 */
public final class KafkaVersionUtils {
    ...
    /**
     * Checks if the current Kafka client version supports share groups (KIP-932). Share groups were
     * introduced in Kafka 4.0.0 (experimental) and became stable in 4.1.0.
     *
    ...
    /**
     * Validates that the provided properties are compatible with the current Kafka version for
     * share group usage.
     *
    ...
    public static void validateShareGroupProperties(Properties props) {
        ...
        if ("share".equals(groupType)) {
            ...
            LOG.info(
                    "Share group support detected and enabled for Kafka version: {}",
                    getKafkaVersion());
        }
    }
    ...
    private static boolean detectShareGroupSupport() {
        try {
            ...
            try {
                Class<?> consumerConfigClass =
                        Class.forName("org.apache.kafka.clients.consumer.ConsumerConfig");
        ...
        try {
            ...
            try {
                Package kafkaPackage =
                        org.apache.kafka.clients.consumer.KafkaConsumer.class.getPackage();
    ...
    private static boolean isVersionAtLeast(String currentVersion, String requiredVersion) {
        ...
        try {
            ...
            LOG.warn(
                    "Failed to compare versions {} and {}: {}",
                    currentVersion,
                    requiredVersion,
                    e.getMessage());
            return false;
