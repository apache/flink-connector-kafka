...
public class TransactionAbortStrategyContextImpl implements TransactionAbortStrategyImpl.Context {
    ...
    /**
     * Transactions that must not be aborted unless the broker reported a later one under the id.
     */
    private final Map<String, CheckpointTransaction> precommittedTransactions;

    /** Aborts the broker-side transaction of a superseded precommitted transactional id. */
    private final Consumer<String> precommittedTransactionAborter;
    ...
    public TransactionAbortStrategyContextImpl(
            ...
            Consumer<String> precommittedTransactionAborter) {
        ...
        this.precommittedTransactionAborter =
                checkNotNull(
                        precommittedTransactionAborter,
                        "precommittedTransactionAborter must not be null");
    }
    ...
    public boolean isPrecommittedTransactionSuperseded(String transactionalId) {
        ...
        if (superseded) {
            LOG.warn(
                    "Restored transaction {} recorded producer id {} and epoch {}, but the broker"
                            + " reported producer id {} and epoch {} in state {}; the id was reused"
                            + " after the checkpoint, the committer's commit of the restored"
                            + " transaction is fenced, and the reported transaction is aborted.",
                    transactionalId,
    ...
    public void abortSupersededPrecommittedTransaction(String transactionalId) {
        precommittedTransactionAborter.accept(transactionalId);
    }
